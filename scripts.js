...
function loadDoc() { //Loading the XML File
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (document.getElementById('cd-table')) {
                document
                    .getElementById("cd-table")
                    .innerHTML = "<h2 class='text-center'>XML File Could Not be Loaded</h2>"; //Shows when fails to load the XML File
            }
...
function myFunction(xml) { //Creating the table from Loaded XML file
    ...
    var table = "<tr><th>Title</th><th>Artist</th><th>Country</th><th>Company</th><th>Price</th" +
            "><th>Year</th><th colspan='2' class='text-center'>Modify</th></tr>";
    ...
    if (x.length != 0) {
        ...
        for (i = 0; i < x.length; i++) {
            table += "<tr><td>" + x[i]
                ...
                .childNodes[0]
                .nodeValue + "</td><td>" + x[i]
                .getElementsByTagName("company")[0]
                .childNodes[0]
                .nodeValue + "</td><td>" + x[i]
                ...
                .childNodes[0]
                .nodeValue +
                        "</td><td><button class='btn btn-danger' onclick=\"removeXML('cd'," + i + ");\"" +
                    ">Delete</button></td></tr>";
        }
        if (document.getElementById('cd-table')) {
            document
        ...
        document
            .getElementById("cd-table")
            .innerHTML = "<h2 class='text-center'>No Data to show</h2>"; //Show no data in the XML File
    }
...
function showResultByCountry(param) {
    var xml = xhttp.responseXML;
    console.log(xml);
    var txt = "";
    var txt2 = "";
    path = "/catalog/cd/title";
    path2 = "/catalog/cd/country";
    if (xml.evaluate) {
        ...
        while (result) {
            if (result2.childNodes[0].nodeValue == param) {
                txt += "<li>" + result
                    .childNodes[0]
                    .nodeValue + "</li>";
                txt2 += "<li>" + result2
        ...
        document
            .getElementById('titleList')
            .innerHTML = "<ul>" + txt + "</ul>";
        document
            .getElementById('countryList')
            .innerHTML = "<ul>" + txt2 + "</ul>";
    }
}
